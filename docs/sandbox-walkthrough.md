...
A Stripe-like `payment.succeeded` event is mapped to payment-audit and Slack-style operations. Replaying the same event proves duplicate detection through the idempotency key. In Mock Job 01, this gives the payment-side intake proof that is later paired with downstream ops ledgers and exception reporting in `sheets-airtable-sync`.
...
The script starts the local API, sends all synthetic fixtures, saves pretty JSON responses under `examples/api-responses/`, verifies the saved response contract, and shuts the server down. If the configured port is already occupied, it exits before deleting or regenerating artifacts so a stale server cannot contaminate the verification run.
